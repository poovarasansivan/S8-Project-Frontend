...
function Pipelinedcompany() {
  const [slotdetails, setslotstatus] = useState([]);
    ...
    const fetchSlotDetails = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:8080/protected/get-individual-psslotdata/${rollno}`,
          {
            headers: {
  ...
  const sortedEmployees = [...slotdetails].sort((a, b) => {
