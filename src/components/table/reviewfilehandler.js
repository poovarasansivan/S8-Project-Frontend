...
export default function PSFileHandler({ onImport, data }) {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Map imported data to match Fullstackreviewdata structure
          const transformedData = jsonData.map((item) => ({
            rollno: item.rollno || '',
            name: item.name || '',
            reviewer_name: item.reviewer_name || '',
            reviewer_email: item.reviewer_email || '',
            reviewer_contact: item.reviewer_contact || '',
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'FullstackReviewData');
    ...
    link.href = url;
    link.download = 'FullstackReviewData.xlsx';
    link.click();
